var buildFullURL = function(rootURL , paramsObj){
  var params = buildQueryString( paramsObj) || ''
  var fullURL = rootURL + params ;
  return fullURL
}

var buildQueryString = function(paramsObject) {
  if ( paramsObject && typeof paramsObject === 'object' )
  
  var qryBuilderStr = '?'
  var proopList = []
  for (var proop in paramsObject){
    proopList.push(proop)
  }
  for (var i = 0; i < proopList.length; i++){
  
    qryBuilderStr += proopList[i] + "=" + paramsObject[proopList[i]]
    
    if(i !== proopList.length - 1 ){ 
      qryBuilderStr += "&"
    }

  }
  return qryBuilderStr
}
